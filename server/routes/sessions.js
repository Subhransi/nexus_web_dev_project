...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const populatedSession = await Session.findById(newSession._id).populate('subject').populate('todo');
...
router.put('/:id', async (req, res) => {
  ...
  if (req.body.todo !== undefined) session.todo = req.body.todo;

  const updatedSession = await session.save();
  const populatedSession = await Session.findById(updatedSession._id).populate('subject').populate('todo');
